...
RU_TEAMS = {
    ...
    "DR Congo": "ДР Конго", "Congo DR": "ДР Конго", "Cape Verde": "Кабо-Верде", "Curacao": "Кюрасао",
    "Curaçao": "Кюрасао",
...
def _fix_groups_table(cur):
    ...
    try:
        ...
        log.warning("_fix_groups_table read failed: %s", e);
        return 0
    fixed = 0
...
def load_all():
    ...
    try:
        ensure_group_letters()
    except Exception as e:
        log.warning("ensure_group_letters: %s", e)
...
def get_fixtures(start, end=None, limit=30):
    ...
    sql += " ORDER BY match_date,home LIMIT %s";
    params.append(limit)
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "SELECT match_date,home,away,kickoff_utc FROM wc2026_fixtures WHERE kickoff_utc IS NOT NULL")
...
def _msk_date(d, h, a):
    ...
    if kt:
        try:
            return kt.astimezone(timezone(timedelta(hours=3))).date()
        except Exception:
            pass
    return d
...
async def _ingest_now(timeout=120):
    ...
    try:
        ...
        if r.returncode != 0:
            log.warning("ingest_now rc=%s stderr=%s", r.returncode, (r.stderr or "")[:300]);
            return False
        return True
    except Exception as e:
        log.warning("ingest_now exception: %s", e);
        return False
    finally:
        _UPDATING = False
...
def get_snapshot(key):
    ...
    try:
        ...
        log.warning("get_snapshot(%s): %s", key, e);
        return None
...
def recompute_prediction_stats():
    ...
    for d, home, away, hs, as_, host in finished:
        ...
        for _t in (home, away):
            try:
                team_dates.setdefault(_wc_canon(_t), []).append(str(d)[:10])
            except Exception:
                pass
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                ...
                cur.execute(
                    "SELECT match_date,home,away,advance_winner FROM wc2026_fixtures WHERE advance_winner IS NOT NULL")
                ...
                for d, home, away, pcode, psh, psa in rows:
                    ...
                    if r is None:
                        cur.execute(
                            'UPDATE wc2026_predictions SET pred_mode=%s WHERE match_date=%s AND home=%s AND away=%s',
                            (mode, d, home, away))
                        upd += cur.rowcount;
                        continue
                    hs, as_ = r
                    ...
                    try:
                        exact = (psh is not None and psa is not None and int(psh) == hs and int(psa) == as_)
                    except Exception:
                        exact = None
                    ok = (pcode == actual)
                    if actual == 'D':
                    ...
                    cur.execute(
                        'UPDATE wc2026_predictions SET actual_home=%s,actual_away=%s,actual_code=%s,correct=%s,exact_correct=%s,pred_mode=%s,resolved_at=COALESCE(resolved_at,now()) WHERE match_date=%s AND home=%s AND away=%s',
                        (hs, as_, actual, ok, exact, mode, d, home, away))
                    upd += cur.rowcount
            conn.commit()
    except Exception as e:
        ...
        if os.path.exists("wc2026_squad_values.csv"):
            with open("wc2026_squad_values.csv", encoding="utf-8-sig") as fh:
                for row in csv.DictReader(fh):
                    try:
                        val[row["team"]] = float(row["value_eur"])
                    except:
                        pass
    except Exception as e:
        log.warning("squad_values csv: %s", e)
    try:
...
def squad_value(team):
    ...
    try:
        return float(SQUAD_VALUE.get(k))
    except:
        return None
...
def sensation_note(home, away, p_h, p_d, p_a, o1=None, ox=None, o2=None):
    ...
    if vu and vo and vu > vo * 1.25:
        return "\U0001f31f \u0421\u043e\u0441\u0442\u0430\u0432 " + ru_team(
            under) + " \u0434\u043e\u0440\u043e\u0436\u0435 \u2014 \u0432\u043e\u0437\u043c\u043e\u0436\u043d\u0430 \u0441\u0435\u043d\u0441\u0430\u0446\u0438\u044f"
...
def predict_1x2(home, away, neutral=False, _elo=None):
    ...
    eh = _elo_get(src, home);
    ea = _elo_get(src, away)
...
def predict_natural(p_h, p_d, p_a, home, away, ko=False):
    ...
    if mp >= 0.58 and absd >= 180:
        ...
        c_e, c_l = "\u26a1", "средняя"; expl = f"небольшое преимущество {ru_team(fav)}"
    else:
...
def fmt_detail(d, home, away, host, o1=None, ox=None, o2=None, kt=None):
    ...
    if o1 and ox and o2:
        try:
            lines += ["",
                      f"\U0001f4b0 <b>Коэффы:</b> <code>1 \u2014 {float(o1):.2f}   X \u2014 {float(ox):.2f}   2 \u2014 {float(o2):.2f}</code>"]
        except:
            pass
    return "\n".join(lines)
...
async def cmd_baseline(u, c):
    ...
    if not probs:
        await u.message.reply_text("\u26a0\ufe0f Baseline не загружен. /reload");
        return
...
async def cmd_forecast(u, c):
    ...
    for letter in "ABCDEFGHIJKL":
        ...
        for i, t in enumerate(teams, 1):
            ...
            mark = "\U0001f947" if i == 1 else (
                "\U0001f948" if i == 2 else ("\U0001f949" if i == 3 else "\u25aa\ufe0f"))
            st1.append(
...
async def cmd_next(u, c):
    ...
    if c.args:
        try:
            n = min(int(c.args[0]), MAX_NEXT)
        except:
            pass
    rows = get_fixtures(date.today(), limit=n)
    if not rows:
        await u.message.reply_text("\u26bd Матчей не найдено.");
        return
...
async def cmd_match(u, c):
    ...
    if _re.fullmatch(r"\d{4}-\d{2}-\d{2}", args[-1]):
        try:
            target_date = date.fromisoformat(args.pop())
        except Exception:
            pass
    if len(args) < 2:
        await u.message.reply_text("❌ Укажи две команды.", parse_mode=ParseMode.HTML);
        return
    team_a = team_b = None
...
async def cmd_team(u, c):
    if not c.args:
        await u.message.reply_text("Использование: /team Аргентина  (или /team Argentina)");
        return
    name = " ".join(c.args)
    all_t = list(ELO.keys())
    team = None
    ...
    if not team:
        rv = {ru_team(t).lower(): t for t in all_t}
        if name.strip().lower() in rv:
            team = rv[name.strip().lower()]
        else:
            cand = difflib.get_close_matches(name, all_t, n=1, cutoff=0.6)
            if cand:
                team = cand[0]
    ...
    if not team:
        await u.message.reply_text(f"\u274c Команда \u00ab{esc(name)}\u00bb не найдена.", parse_mode=ParseMode.HTML);
        return
...
async def cmd_group(u, c):
    if not c.args:
        await u.message.reply_text("Использование: /group A");
        return
    letter = c.args[0].upper()
    if letter not in "ABCDEFGHIJKL" or len(letter) != 1:
        await u.message.reply_text("Группы: A\u2013L. Например: /group F");
        return
    teams = get_group_teams(letter)
    if not teams:
        await u.message.reply_text("\u274c Группа не найдена.");
        return
    probs = BASELINE.get("tournament_probs", {})
    teams = group_ranked(letter)
...
async def cmd_stats(u, c):
    ...
    if _s2["resolved"]:
        ...
        pm = _s2["by_mode"].get("prematch", (0, 0));
        lv = _s2["by_mode"].get("live", (0, 0))
        grp = _s2["by_stage"].get("group", (0, 0));
        ko = _s2["by_stage"].get("knockout", (0, 0))
        out = [
            "\U0001f3af <b>\u0414\u0412\u0410 \u0422\u0420\u0415\u041a\u0410 \u041f\u0420\u041e\u0413\u041d\u041e\u0417\u041e\u0412</b>",
            SEP, "",
            "<b>\u0414\u0432\u0430 \u0442\u0440\u0435\u043a\u0430 \u043f\u0440\u043e\u0433\u043d\u043e\u0437\u043e\u0432:</b>",
            _ln("\U0001f52e \u0417\u0430\u0440\u0430\u043d\u0435\u0435 (\u0431\u0435\u0437 \u0444\u043e\u0440\u043c\u044b)",
                pm),
...
async def cmd_diff(u, c):
    ...
    a = get_snapshot(key_a);
    b = get_snapshot(key_b)
    if not a:
        await u.message.reply_text(
    ...
    if not b:
        await u.message.reply_text(
            ...
            parse_mode=ParseMode.HTML);
        return
    pa = a.get("tournament_probs", {});
    pa = pa if isinstance(pa, dict) else {}
    pb = b.get("tournament_probs", {});
    ...
    if champ_a or champ_b:
        if champ_a == champ_b:
            lines += ["",
                      f"\U0001f3c6 Модальный чемпион не изменился: <b>{esc(ru_team(champ_a) if champ_a else '?')}</b>"]
        else:
            lines += ["",
                      f"\U0001f3c6 Чемпион (модель): <b>{esc(ru_team(champ_a) if champ_a else '?')}</b> \u2192 <b>{esc(ru_team(champ_b) if champ_b else '?')}</b>"]
    gpa = a.get("group_positions", {});
    gpa = gpa if isinstance(gpa, dict) else {}
    gpb = b.get("group_positions", {});
    gpb = gpb if isinstance(gpb, dict) else {}
    if gpa and gpb:
        gshifts = []
...
async def cmd_update(u, c):
    if not is_admin(u.effective_user.id):
        ...
        return
    if _UPDATING:
        await u.message.reply_text("\u23f3 Обновление уже выполняется — подождите немного.");
        return
    ch = os.environ.get("CHANNEL_ID", "")
    api_key = os.environ.get("FOOTBALL_DATA_API_KEY", "")
    if not api_key:
        await u.message.reply_text("\u274c FOOTBALL_DATA_API_KEY не задан.");
        return
    await u.message.reply_text("\U0001f504 <b>Обновление…</b>\n\u23f3 Шаг 1/2: подгружаю результаты",
                               parse_mode=ParseMode.HTML)
    env = os.environ.copy()
    try:
        ...
        if r1.returncode != 0:
            ...
            low = err.lower()
            if ("api_get failed" in err) or ("remote end closed" in low) or ("urlerror" in low) or (
                    "timed out" in low) or ("connection" in low) or ("http error" in low):
        ...
        await u.message.reply_text(f"\u274c Исключение: {esc(str(e))}", parse_mode=ParseMode.HTML);
        return
    await u.message.reply_text(
        "\u2705 Результаты загружены.\n\u23f3 Обновляю таблицу и прогнозы\u2026 (базовый прогноз заморожен)",
        parse_mode=ParseMode.HTML)
    load_all()
...
def _parse_msk_day(c):
    ...
    if c.args:
        ...
        if a in ("сегодня", "today"):
            md = datetime.now(MSK_TZ).date()
        elif a in ("завтра", "tomorrow"):
            md = datetime.now(MSK_TZ).date() + timedelta(days=1)
        elif a in ("вчера", "yesterday"):
            md = datetime.now(MSK_TZ).date() - timedelta(days=1)
        else:
            try:
                md = date.fromisoformat(a)
            except Exception:
                pass
    return md
...
async def cmd_post_preview(u, c):
    ...
    md = _parse_msk_day(c)
    await u.message.reply_text(
        f"\U0001f50e <b>ПРЕДПРОСМОТР за {fmt_date_ru(md)}</b> — в канал НЕ отправлено.\nОпубликовать в канал: <code>/post_day {md.isoformat()}</code>",
        parse_mode=ParseMode.HTML)
...
async def cmd_post_day(u, c):
    ...
    await u.message.reply_text(
        f"\u2705 Опубликовано в канал за {fmt_date_ru(md)}." if ok else f"\u26bd На {fmt_date_ru(md)} матчей нет.",
        parse_mode=ParseMode.HTML)
...
async def job_results(ctx):
    ...
    if total:
        _s = get_accuracy_stats();
        c_all, r_all = _s["correct"], _s["resolved"]
...
async def _legacy_elo_post(u, c):
    ...
    ch = os.environ.get("CHANNEL_ID", "")
    if not ch:
        await u.message.reply_text("\u26a0\ufe0f CHANNEL_ID не задан");
        return
    ...
    if changed:
        ...
        if dns:
            ...
            for t, pp, nn, dd in dns:
                L.append("   <b>%s</b>  <code>%.0f</code>→<code>%.0f</code>  <b>%s</b>" % (rt(t), pp, nn, _elo_sg(dd)))
        pre_rank = {t: i for i, (t, _v) in
                    enumerate(sorted(((t, pp) for t, pp, nn, dd in rows), key=lambda kv: kv[1], reverse=True), 1)}
        now_rank = {t: i for i, (t, _v) in
                    enumerate(sorted(((t, nn) for t, pp, nn, dd in rows), key=lambda kv: kv[1], reverse=True), 1)}
        movers = []
        for t in cur:
        ...
        L.append("")
        L.append(
            "ℹ️ Пока никто не изменился. После первых матчей здесь появятся те, кто растёт и падает, и рывки в рейтинге.")
    L.append("")
...
def _fmt_elo_team(team, base, cur):
    ...
    if hist:
        ...
        for d_, opp, gf, ga, before, after, delta in hist:
            L.append("   vs %s %s:%s  <code>%.0f</code>→<code>%.0f</code>  <b>%s</b>" % (rt(opp), gf, ga, before, after,
                                                                                         _elo_sg(delta)))
        L.append("")
...
def get_all_finished():
    try:
        ...
        log.warning("get_all_finished: %s", e);
        return []
...
def get_match_elo_change(d, home, away):
    ...
    try:
        ...
        log.warning("get_match_elo_change: %s", e);
        return None
...
def get_elo_baseline():
    ...
    try:
        ...
        log.warning("get_elo_baseline: %s", e);
        return {}
...
def get_current_elo_db():
    ...
    try:
        ...
        log.warning("get_current_elo_db: %s", e);
        return {}
...
def get_recent_finished(limit=20):
    try:
        ...
        log.warning("get_recent_finished: %s", e);
        return []
...
def compute_real_standings(letter, finished=None):
    ...
    for d, home, away, hs, as_, host in (finished if finished is not None else get_all_finished()):
        ht = canon2team.get(_c(home));
        at = canon2team.get(_c(away))
...
def group_advance_ranked(letter):
    teams = get_group_teams(letter)
    probs = BASELINE.get("tournament_probs", {})
    teams.sort(key=lambda t: (BASELINE.get("mean_points", {}).get(t, 0.0), probs.get(t, {}).get("P_R32", 0)),
               reverse=True)
...
async def cmd_schedule(u, c):
    ...
    show_all = False
    if c.args:
        if c.args[0].lower() in ("all", "все", "всё"):
            show_all = True
        else:
            try:
                n = min(int(c.args[0]), 40)
            except:
                pass
    rows = get_fixtures(date.today(), limit=200 if show_all else n)
    if not rows:
        await u.message.reply_text("📅 Ближайших матчей не найдено.", parse_mode=ParseMode.HTML);
        return
    ...
    for msk_d, msk_dt, d, home, away, host in items:
        if msk_d != cur_day:
            cur_day = msk_d;
            lines.append("");
            lines.append(f"📆 <b>{fmt_date_ru(msk_d)}</b>")
        grp = get_team_group(home) or "?"
...
async def cmd_pens(u, c):
    ...
    if not is_admin(u.effective_user.id):
        ...
        return
    if len(c.args) < 2:
        await u.message.reply_text("Использование: /pens 2026-06-29 Парагвай");
        return
    try:
        d = date.fromisoformat(str(c.args[0]).strip())
    except Exception:
        await u.message.reply_text("\u274c Дата в формате ГГГГ-ММ-ДД.");
        return
    ...
    try:
        with get_conn() as conn:
            with conn.cursor() as cur:
                cur.execute("ALTER TABLE wc2026_fixtures ADD COLUMN IF NOT EXISTS advance_winner TEXT")
                cur.execute(
                    "SELECT home,away FROM wc2026_fixtures WHERE match_date=%s AND home_score IS NOT NULL AND home_score=away_score",
                    (d,))
                rows = cur.fetchall()
            conn.commit()
    except Exception as e:
        await u.message.reply_text(f"\u274c Ошибка БД: {esc(str(e))}", parse_mode=ParseMode.HTML);
        return
    target = None
    for h, a in rows:
        for t in (h, a):
    ...
    if not target:
        await u.message.reply_text("\u274c Ничейный матч с этой командой за эту дату не найден.");
        return
    h, a, w = target
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute("UPDATE wc2026_fixtures SET advance_winner=%s WHERE match_date=%s AND home=%s AND away=%s",
                        (w, d, h, a))
        conn.commit()
...
async def cmd_tour(u, c):
    ...
    if lines is None:
        await u.message.reply_text(
            f"\u26bd По «{TOURS[idx][0]}» пока нет сыгранных матчей.", parse_mode=ParseMode.HTML);
        return
    lines.append("\U0001f916 @wc2026_football_bot")
...
async def cmd_posttour(u, c):
    ...
    if idx is None:
        await u.message.reply_text(
            "\u2754 Укажи тур: /posttour 4 (1/16), /posttour 9 (финал) и т.д.", parse_mode=ParseMode.HTML);
        return
    lines = _tour_results_lines(idx)
    if lines is None:
        await u.message.reply_text(
...
async def cmd_table(u, c):
    letters = [c.args[0].upper()] if (c.args and len(c.args[0]) == 1 and c.args[0].upper() in "ABCDEFGHIJKL") else list(
        "ABCDEFGHIJKL")
    finished = get_all_finished()
    played = len(finished)
    if played:
...
async def cmd_value(u, c):
    ...
    if not bets:
        ...
        lines.append("ℹ️ Нужны коэфф�� (THE_ODDS_API_KEY) и ближайшие матчи.")
    else:
...
def _export_fixtures_to_csv(path="wc2026_fixtures.csv"):
    ...
    определение групп в wc2026_simulate.py (сетку он строит сам)."""
    import csv as _csv
    with get_conn() as conn:
        with conn.cursor() as cur:
            cur.execute(
...
async def _run_auto_update(bot):
    ...
    api_key = os.environ.get("FOOTBALL_DATA_API_KEY", "")
    if not api_key:
        log.info("auto-update skipped: no FOOTBALL_DATA_API_KEY");
        return
    if _UPDATING:
        log.info("auto-update skipped: another update is in progress");
        return
    _UPDATING = True
    ...
    try:
        ...
        if after <= before:
            log.info("auto-update: no new results (have %d)", after);
            return
...
async def cmd_squad(u, c):
    ...
    if not key:
        await u.message.reply_text(
            f"\u041d\u0435 \u043d\u0430\u0448\u0451\u043b \u043a\u043e\u043c\u0430\u043d\u0434\u0443 \u00ab{esc(arg)}\u00bb. \u041e\u0442\u043a\u0440\u043e\u0439 /squad \u0431\u0435\u0437 \u0430\u0440\u0433\u0443\u043c\u0435\u043d\u0442\u0430 \u2014 \u0442\u0430\u043c \u0441\u043f\u0438\u0441\u043e\u043a.");
        return
    v = SQUAD_VALUE.get(key);
    players = SQUADS.get(key, [])
    head = [f"\U0001f3f4 <b>{rt(key)} \u2014 \u0441\u043e\u0441\u0442\u0430\u0432 \u0427\u041c-2026</b>", SEP]
    if v: head.append(
        f"\U0001f4b0 \u0421\u0442\u043e\u0438\u043c\u043e\u0441\u0442\u044c \u0441\u043e\u0441\u0442\u0430\u0432\u0430: <b>\u20ac{v / 1e6:.0f} \u043c\u043b\u043d</b> (Transfermarkt)")
    POSRU = {"GK": "\U0001f9e4 \u0412\u0440\u0430\u0442\u0430\u0440\u0438",
             "DF": "\U0001f6e1 \u0417\u0430\u0449\u0438\u0442\u043d\u0438\u043a\u0438",
             "MF": "\u2699\ufe0f \u041f\u043e\u043b\u0443\u0437\u0430\u0449\u0438\u0442\u0430",
             "FW": "\u26bd \u041d\u0430\u043f\u0430\u0434\u0435\u043d\u0438\u0435"}
    body = []
    for pos in ["GK", "DF", "MF", "FW"]:
        ...
        for pl in grp:
            body.append(
                f"  {pl.get('number', '')}. {esc(_nfkc(pl.get('player', '')))} \u00b7 <i>{esc(_nfkc(pl.get('club', '')))}</i>")
    for p in split_text("\n".join(head + body)):
        await u.message.reply_text(p, parse_mode=ParseMode.HTML)
...
async def cmd_predict_legacy(u, c):
    ...
    if not is_admin(u.effective_user.id):
        await u.message.reply_text("\U0001f512 Только для разработчика.");
        return
    n = 5
    if c.args:
        try:
    ...
    for md, h, a, host, o1, ox, o2 in rows:
        ...
        if nw != lw: flips += 1
        eh_b = baseline_elo.get(h, DEFAULT_ELO);
        ea_b = baseline_elo.get(a, DEFAULT_ELO)
        eh_n = ELO.get(h, DEFAULT_ELO);
        ea_n = ELO.get(a, DEFAULT_ELO)
        lines += [
            "",
    ...
    if flips:
        lines.append(
            f"\u26a0\ufe0f \u0421\u0438\u0441\u0442\u0435\u043c\u0430 \u0438\u0437\u043c\u0435\u043d\u0438\u043b\u0430 \u0444\u0430\u0432\u043e\u0440\u0438\u0442\u0430 \u0432 <b>{flips}/{len(rows)}</b> \u043c\u0430\u0442\u0447\u0430\u0445.")
    else:
        lines.append(
            "\u2705 \u0424\u0430\u0432\u043e\u0440\u0438\u0442 \u043d\u0438 \u0432 \u043e\u0434\u043d\u043e\u043c \u043c\u0430\u0442\u0447\u0435 \u043d\u0435 \u043f\u043e\u043c\u0435\u043d\u044f\u043b\u0441\u044f \u2014 \u0445\u0430\u0440\u0430\u043a\u0442\u0435\u0440 \u043f\u0440\u043e\u0433\u043d\u043e\u0437\u043e\u0432 \u0441\u0442\u0430\u0431\u0438\u043b\u044c\u043d\u044b\u0439.")
    for p in split_text("\n".join(lines)):
        await u.message.reply_text(p, parse_mode=ParseMode.HTML)
...
async def cmd_sim_legacy(u, c):
    ...
    try:
        ...
        if result.returncode != 0:
            ...
            await u.message.reply_text(_sim_fail_msg(err), parse_mode=ParseMode.HTML);
            return
        ...
        lines.append("")
        lines.append(
            "<b>\u0414\u0435\u043b\u044c\u0442\u0430 P(W) \u0421\u0422\u0410\u0420. \u2192 \u041d\u041e\u0412. (\u0442\u043e\u043f-20):</b>")
        lines.append("<pre>")
        lines.append(
            f"{'#':<3}{'\u041a\u043e\u043c\u0430\u043d\u0434\u0430':<20}{'\u0421\u0422\u0410\u0420.':>8}{'\u041d\u041e\u0412.':>8}{'\u0394':>8}")
        ...
        lines.append(
            "\U0001f4dd \u041f\u043e\u0432\u0442\u043e\u0440 \u0431\u0435\u0437 \u0441\u0438\u043c\u0443\u043b\u044f\u0446\u0438\u0438: <code>/compare_top</code>")
        for p in split_text("\n".join(lines)):
            await u.message.reply_text(p, parse_mode=ParseMode.HTML)
    except subprocess.TimeoutExpired:
        await u.message.reply_text(
            "\u274c Sim timeout (10 \u043c\u0438\u043d). \u041f\u043e\u043f\u0440\u043e\u0431\u0443\u0439 \u043c\u0435\u043d\u044c\u0448\u0435 sims.")
    except Exception as e:
        await u.message.reply_text(f"\u274c \u041e\u0448\u0438\u0431\u043a\u0430: <pre>{esc(str(e))}</pre>",
                                   parse_mode=ParseMode.HTML)
...
async def cmd_sim_new(u, c):
    ...
    await u.message.reply_text(
        ...
        "\u2026 2\u20135 \u043c\u0438\u043d\u0443\u0442. \u0411\u0443\u0434\u0435\u0442 \u043f\u043e\u043b\u043d\u044b\u0439 \u0440\u0435\u0439\u0442\u0438\u043d\u0433 \u043a\u043e\u043c\u0430\u043d\u0434.",
        parse_mode=ParseMode.HTML)
    try:
        ...
        if old_modal != new_modal:
            lines.append(
                f"   <i>\u0431\u044b\u043b\u043e: {esc(ru_team(old_modal))} \u2192 \u0441\u0442\u0430\u043b\u043e: {esc(ru_team(new_modal))}</i>")
        lines += [
            "",
        ...
        if old_tp:
            lines += ["",
                      "<b>\u0427\u0442\u043e \u0438\u0437\u043c\u0435\u043d\u0438\u043b\u043e\u0441\u044c \u043e\u0442\u043d\u043e\u0441\u0438\u0442\u0435\u043b\u044c\u043d\u043e \u043f\u0440\u0435\u0434\u044b\u0434\u0443\u0449\u0435\u0433\u043e BASELINE (\u0442\u043e\u043f-10 \u0434\u0432\u0438\u0436\u0435\u043d\u0438\u0439):</b>"]
            deltas = []
            for t, p in tp.items():
            ...
            lines.append("<pre>")
            lines.append(
                f"{'\u041a\u043e\u043c\u0430\u043d\u0434\u0430':<20}{'\u0431\u044b\u043b\u043e':>8}{'\u0441\u0442\u0430\u043b\u043e':>9}{'\u0394':>8}")
        ...
        await u.message.reply_text(
            "\u274c Sim timeout (15 \u043c\u0438\u043d). \u041f\u043e\u043f\u0440\u043e\u0431\u0443\u0439 \u043c\u0435\u043d\u044c\u0448\u0435 sims.")
    except Exception as e:
        await u.message.reply_text(f"\u274c \u041e\u0448\u0438\u0431\u043a\u0430: <pre>{esc(str(e))}</pre>",
                                   parse_mode=ParseMode.HTML)
...
async def cmd_set_live(u, c):
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute("UPDATE wc2026_artifacts SET content=%s, updated_at=NOW() WHERE key='baseline'",
                        (json.dumps(r[0]),))
            conn.commit()
    load_all()
...
async def cmd_snap_del(u, c):
    if not is_admin(u.effective_user.id):
        await u.message.reply_text("🔒 Только для разработчика.");
        return
    if not c.args:
    ...
    if n:
        await u.message.reply_text(f"🗑 Снимок <code>{esc(lbl)}</code> удалён. Проверь: /snapshots",
                                   parse_mode=ParseMode.HTML)
    else:
        await u.message.reply_text(f"❌ Снимок <code>{esc(lbl)}</code> не найден. Список: /snapshots",
                                   parse_mode=ParseMode.HTML)
    ...
    if rounds:
        ...
        for rnd in rounds:
            ...
            for m in rnd.get("matches", []):
                h = ru_team(m.get("home", "?"));
                a = ru_team(m.get("away", "?"))
                w = m.get("winner");
                adv = m.get("adv");
                sc = m.get("score")
                tail = ""
...
def _view_group_lines(data):
    ...
    for letter in "ABCDEFGHIJKL":
        ...
        pair = [t for t in (g2.get(letter) or []) if t in teams][:2]
        rest = [t for t in teams if t not in pair]
        rest.sort(key=lambda t: ((gp.get(t, {}) or {}).get("3", 0.0), (probs.get(t, {}) or {}).get("P_R32", 0)),
                  reverse=True)
        ranked = pair + rest
...
async def cmd_view(u, c):
    ...
    if len(extra) >= 2:
        ...
        if not info:
            ...
            ph = (info.get("p_home") or 0) * 100;
            pd = (info.get("p_draw") or 0) * 100;
            pawy = (info.get("p_away") or 0) * 100
            w = info.get("winner");
            sc = info.get("score", "?");
            adv = info.get("adv")
    ...
    total = data.get("matches_total", 72)
    lines = [
...
async def cmd_compare_top(u, c):
    ...
    if key_a == "live":
        key_a = "baseline"
    elif not key_a.startswith("baseline"):
        key_a = f"baseline_{key_a}"
    if key_b == "live":
        key_b = "baseline"
    elif not key_b.startswith("baseline"):
        key_b = f"baseline_{key_b}"
    ...
    if champ_a or champ_b:
        if champ_a == champ_b:
            ...
            lines += ["",
                      f"🏆 Чемпион (модель): <b>{esc(ru_team(champ_a) if champ_a else '?')}</b> → <b>{esc(ru_team(champ_b) if champ_b else '?')}</b>"]
    ...
    lines += format_shifts(get_top_deltas("P_R32", pa, pb), "Топ изменений: Выход в 1/16 ПЛ��Й-ОФФ")
    lines += format_shifts(get_top_deltas("1", gpa, gpb, is_group=True, min_delta=0.015),
                           "Топ изменений: 1-Е МЕСТО В ГРУППЕ")
...
HELP_ADMIN = "\n".join([
    ...
    "/set_live &lt;версия&gt; — назначить выбранный снимок текущ��м прогнозом (live) для всех. Пример: <code>/set_live prematch_FROZEN</code>",
...
def main():
    ...
    handlers = [
        ...
        ("post_preview", cmd_post_preview), ("post_day", cmd_post_day), ("post_today", cmd_post_day),
        ("post_forecast", cmd_post_forecast),
        ...
        try:
            ...
            if uid is not None and is_admin(uid):
                ...
                await msg.reply_text(
                    "❌ Внутренняя ошибка при обработке запроса — уже залогировано, попробуйте ещё раз.")
        except Exception:
            pass
    ...
    if app.job_queue:
        ...
        if results_on:
            # Авто-итоги: ОДИН пост на тур (включая раунды плей-офф), как только
            # все матчи тура сыграны. RESULTS_POST=0 отключает полностью.
